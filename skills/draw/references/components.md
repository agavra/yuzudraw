...
- Use `textOnBorder` for container labels — put the label as the rect's own text (e.g., `rect "Platform" ... textOnBorder`), not as a separate `text` element.
...
|---|---|---|---|
| Outer boundary | `style double` | none | Use `textOnBorder` for label |
...
```dsl
rect "Platform" id platform at 2,1 size 76x24 style double textOnBorder shadow light

rect "Ingestion" id ingest at 6,4
rect "Processor" id proc below ingest gap 3
rect "Query API" id query at 50,16

rect "Data Layer" id datalayer at 30,3 size 40x12 style double textOnBorder
...
```dsl
rect "App" id app at 2,1 size 64x16 style double textOnBorder shadow light
...
rect "" id frame at 2,8 size 54x6 style double textOnBorder
...
rect "System" id sys at 2,6 size 60x14 style double textOnBorder shadow light
